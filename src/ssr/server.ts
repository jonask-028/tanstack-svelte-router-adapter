 ...
 */
export type AppComponent = Component<{ router: AnyRouter }>;
 ...
 */
export function renderRouterToString({
  router,
  responseHeaders,
  App = RouterServerComponent,
}: RenderRouterToStringOptions): Response {
 ...
 */
export function renderRouterToStream({
  router,
  responseHeaders,
  App = RouterServerComponent,
}: RenderRouterToStreamOptions): Response {
  const fullHtml = renderDocument(router, App);
...
function getStatusCode(router: AnyRouter): number {
  return (router.state as { statusCode?: number }).statusCode ?? 200;
}
 ...
 */
function renderDocument(router: AnyRouter, App: AppComponent): string {
...
function renderTag(tag: ManagedTag): string {
 ...
 */
function collectAssetTags(router: AnyRouter): string {
  const manifest = (
    router as {
      ssr?: {
        manifest?: { routes?: Record<string, { assets?: Array<ManagedTag> }> };
      };
    }
  ).ssr?.manifest;
  ...
  for (const match of router.state.matches) {
    ...
    if (!routeManifest?.assets) continue;
 ...
 */
function getDehydrationScripts(router: AnyRouter): string {
  const serverSsr = (
    router as {
      serverSsr?: {
        setRenderFinished?: () => void;
        takeBufferedScripts?: () => ManagedTag | undefined;
